...
class Logger:
    ...
    def log_hparams(self, ENVIRONMENT, SEED, model, LEARNING_RATE, DISCOUNT_FACTOR, ENTROPY_COEFF, activation_func,
                    normalize_rewards, normalize_advantages, clip_grad, notes, display=True):
    ...
    def log_epoch(self, epoch_no, epoch_info):
        ...
        self.writer.add_scalar("Metrics/Timesteps_per_Epoch", epoch_info['epoch_timesteps'], epoch_no)
        self.writer.add_scalar("Metrics/Episodes_per_Epoch", epoch_info['num_episodes'], epoch_no)
        self.writer.add_scalar('Time/Last_episode_time', time.perf_counter() - self.last_episode_time, epoch_no)
        self.writer.add_scalar('Metrics/Raw_Reward', epoch_info['avg_ep_raw_rew'], epoch_no)
        self.last_episode_time = time.perf_counter()
